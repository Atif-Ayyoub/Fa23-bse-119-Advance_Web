...
namespace SMS
{
    public partial class class02 : Form
    {
        public class02()
        {
            InitializeComponent();
        }
        private void LoadPrepStudents()
        {
            try
            {
                ...
                // SQL query to join Student and Class tables for "Prep" class
                string query = @"
        SELECT 
            s.Student_ID, 
            s.Student_Name, 
        ...
        WHERE 
            c.Class_Name = 'Prep';";
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    dataGridViewPrep.DataSource = dataTable; // Make sure dataGridViewPrep is your DataGridView name
                }
            }
            catch (Exception ex)
            {
        ...
        private void LoadClassDetails()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    // Query to fetch the teacher's first name for "Prep" class
                    string queryTeacher = @"
            SELECT t.First_Name 
            FROM Class c
            INNER JOIN Teacher t ON c.Teacher_ID = t.Teacher_ID
            WHERE c.Class_Name = 'Prep';";
                    ...
                    // Query to fetch total students for "Prep" class
                    string queryTotalStudents = @"
            SELECT COUNT(*) 
            FROM Student 
            WHERE Class_ID = (SELECT Class_ID FROM Class WHERE Class_Name = 'Prep');";
        ...
        private void dataGridViewNursery_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void class02_Load(object sender, EventArgs e)
        {
            LoadClassDetails();
            LoadPrepStudents();
        }
